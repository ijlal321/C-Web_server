...
void broadcast(const char *msg, size_t len, struct ws_manager * ws_mgr) {
...
int ws_connect(const struct mg_connection *conn, void *cbdata) {
    struct ws_manager * ws_mgr = (struct ws_manager *)cbdata;
...
int ws_manager_init(struct ws_manager * ws_mgr){
    memset(ws_mgr, 0, sizeof(struct ws_manager));
    return 0;
}

int ws_manager_destroy(struct ws_manager * ws_mgr){
